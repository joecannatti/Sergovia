...
describe Transcriber do
  ...
  describe "#theoretical_voicings" do
    ...
    it "retuns the only way to playable way to voice a low power chord and the one unplayable way" do
      Transcriber.new.theoretical_voicings("e2,b2").should eq [[Note.new(string: 6, fret: 0, pitch: :e2), Note.new(string: 5, fret: 2, pitch: :b2)],
                                                          [Note.new(string: 6, fret: 0, pitch: :e2), Note.new(string: 6, fret: 7, pitch: :b2)]]
    end

    it "low a power chord has 6 possibilities" do
      Transcriber.new.theoretical_voicings("a2,e3").count.should eq 6
    end
  end
  ...
  describe "#voicings" do
    ...
    it "removes voicings with 2 notes on the same string" do
      Transcriber.new.voicings("a2,e3").count.should eq 4
    end
